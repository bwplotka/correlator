...
func (ins *instrumentationTripperware) WrapRoundTripper(targetName string, next http.RoundTripper) http.RoundTripper {
	...
	requestDuration := promauto.With(reg).NewHistogramVec(
		prometheus.HistogramOpts{Name: "http_client_request_duration_seconds", Help: "Tracks the latencies for HTTP requests.", Buckets: ins.buckets},
		[]string{"method", "code"},
	)
	requestsTotal := promauto.With(reg).NewCounterVec(
		prometheus.CounterOpts{Name: "http_client_requests_total", Help: "Tracks the number of HTTP requests."},
		[]string{"method", "code"},
	)
	requestsInFlight := promauto.With(reg).NewGauge(
		prometheus.GaugeOpts{Name: "http_client_requests_inflight", Help: "Tracks the number of HTTP requests currently in flight."},
	)

	base := promhttp.InstrumentRoundTripperInFlight(requestsInFlight, promhttp.RoundTripperFunc(func(req *http.Request) (*http.Response, error) {
		now := time.Now()
		...
		// If we find a TraceID we'll expose it as Exemplar.
